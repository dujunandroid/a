...
//		TencentWeibo.NAME, QZone.NAME, NetEaseMicroBlog.NAME };
	...
	private void init(final Context context) {
		...
		new Thread(){
			public void run() {
				...
				//过滤掉不用的logo//////////////////////////////////////////////
//				Platform[] lst=new Platform[platformList.length-allName.length];
//				int temp=0;
//				boolean bool=true;
//				for (int i = 0; i < platformList.length; i++) {
//					String name=platformList[i].getName();				
//					for (int j = 0; j < allName.length; j++) {
//						if(name.equals(allName[j])){
...
//				platformList=lst;
	...
	private static class PlatformAdapter extends ViewPagerAdapter {
		...
		public PlatformAdapter(PlatformGridView platformGridView) {
			...
			System.out.println(hiddenPlatforms);
			if (platforms != null) {
